...
.equ DEFAULT_CLOCK_CYCLES, 100000000
.equ ENABLE_INTERRUPT_TIMER, 0x7
.equ TIMER_CTL_ENABLE, 0x1

  movia r11,ADDR_JP1PORT_DIR
  movia r3,0b1111000
  stwio r3,0(r11)
  # Enable interrupts on 4 LSB pins 

  movia r11,IRQ_JP1PA
  wrctl ctl3,r11   # Enable bit 11 - button interrupts on Nios II 

     # Enable global Interrupts on Nios II 
...
DONE_IMAGE_LOOP:                   # If no samples in FIFO, go back to start
 ldwio r6,0(r5)
 stwio r6,8(r11)      # Echo to left channel
 stwio r6,12(r11)
 addi r5, r5, 4
 bgt r5, r19, END_AUDIO
 br DONE_IMAGE_LOOP
END_AUDIO:
...
#0 is set to read and 1 is set to write
  movia r11,ADDR_JP1PORT_IE
  movi  r13,0xf
  stwio r13,0(r11)  # Enable interrupts on 4 LSB pins 

  movia r11,IRQ_JP1PA
  wrctl ctl3,r11   # Enable bit 11 - button interrupts on Nios II 

  movia r11,1
  wrctl ctl0,r1   # Enable global Interrupts on Nios II 
...
myISR:
  ...
  #PS2 Interrupt
    ...
    movi r11, 0b0010000000
    beq r13,r11, PS_2_INTERRUPT
...
#  call turnLeftFunction
  ...
  movi r11, 1
  beq r21, r11, RETURN
  movia r11, TIMER_ADDRESS  # r2 contains the address for the timer 
  ...
  stwio r3, 4(r11)    # Start the timer with interrupt
  movi r21, 1
...
#disable_device_interrupts:
  ...
  movia r11,0
  wrctl ctl0,r11   # Enable global Interrupts on Nios II 
